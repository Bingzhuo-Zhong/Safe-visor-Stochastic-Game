"""
This script defines the structure of DNNs, e.g. Actor-critic networks and actor-critic target networks
"""
...
from rein.utils import OrnsteinUhlenbeckActionNoise
import numpy as np
...
class BaseAgent:
    ...
    def build_critic(self, name):
        ...
        input_observations = Input(shape=self.shape_observations, name=name + 'observations_input', dtype=tf.float16)
        input_action = Input(shape=(self.shape_action,), name=name + 'action_input', dtype=tf.float16)

        input_concat = tf.concat([input_observations, input_action], 1, name=name + 'input_concat')
        ...
        model_critic = Model(inputs=[input_observations, input_action], outputs=output_value,
                             name=name + 'critic')
    ...
    def build_actor(self, name):
        ...
        dense1_actor = Dense(self.params.actor_dense1, activation='relu', name=name + 'dense1_actor')(
            input_observations)
    ...
    def get_exploration_action(self, observations):
        ...
        observations = tf.expand_dims(observations, axis=0)  # add batch dim
        action = self.actor(observations).numpy().squeeze()  # squeeze to kill the batch
